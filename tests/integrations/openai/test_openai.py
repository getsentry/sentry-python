...
def test_openai_cache_tokens_prompt_tokens_details(sentry_init, capture_events):
    """Test that cached tokens are tracked from prompt_tokens_details (OpenAI standard)."""
    sentry_init(
        integrations=[OpenAIIntegration(include_prompts=True)],
        traces_sample_rate=1.0,
        send_default_pii=True,
    ...
    # Create a response with cached tokens in prompt_tokens_details
    ...
    client.chat.completions._post = mock.Mock(return_value=mock_response)

    with start_transaction(name="openai"):
        client.chat.completions.create(
            model="gpt-4o",
    ...
    # Check cached tokens from prompt_tokens_details
    assert span["data"][SPANDATA.GEN_AI_USAGE_INPUT_TOKENS_CACHED] == 80
    # OpenAI doesn't track cache writes, so this should not be present
    assert SPANDATA.GEN_AI_USAGE_INPUT_TOKENS_CACHE_WRITE not in span["data"]


def test_openai_cache_tokens_input_tokens_details_compat(sentry_init, capture_events):
    """Test that cached tokens are tracked from input_tokens_details (compatibility)."""
    sentry_init(
        integrations=[OpenAIIntegration(include_prompts=True)],
        traces_sample_rate=1.0,
        send_default_pii=True,
    ...
    # Create a response with cached tokens in input_tokens_details (for compatibility)
    mock_response = ChatCompletion(
        id="chatcmpl-test",
        model="gpt-4o",
        object="chat.completion",
    ...
    # Add input_tokens_details with cached_tokens (compatibility path)
    mock_response.usage.input_tokens_details = {"cached_tokens": 120}
    ...
    assert span["data"][SPANDATA.GEN_AI_USAGE_INPUT_TOKENS] == 150
    assert span["data"][SPANDATA.GEN_AI_USAGE_OUTPUT_TOKENS] == 20
    # Check cached tokens from input_tokens_details
    assert span["data"][SPANDATA.GEN_AI_USAGE_INPUT_TOKENS_CACHED] == 120


def test_openai_reasoning_tokens_compatibility(sentry_init, capture_events):
    """Test that reasoning tokens are tracked from both output_tokens_details and completion_tokens_details."""
    sentry_init(
        integrations=[OpenAIIntegration(include_prompts=True)],
        traces_sample_rate=1.0,
        send_default_pii=True,
    ...
    # Create a response with reasoning tokens in completion_tokens_details
    mock_response = ChatCompletion(
        id="chatcmpl-test",
        model="o1-preview",
        object="chat.completion",
    ...
    # Add completion_tokens_details with reasoning_tokens (OpenAI standard)
    mock_response.usage.completion_tokens_details = {"reasoning_tokens": 30}
    ...
    # Check reasoning tokens from completion_tokens_details
    assert span["data"][SPANDATA.GEN_AI_USAGE_OUTPUT_TOKENS_REASONING] == 30


def test_openai_no_cache_tokens(sentry_init, capture_events):
    ...
    # Response without any cache or reasoning token details
    mock_response = ChatCompletion(
        id="chatcmpl-test",
        model="gpt-3.5-turbo",
        object="chat.completion",
    ...
    assert span["data"][SPANDATA.GEN_AI_USAGE_INPUT_TOKENS] == 50
    assert span["data"][SPANDATA.GEN_AI_USAGE_OUTPUT_TOKENS] == 25
    assert span["data"][SPANDATA.GEN_AI_USAGE_TOTAL_TOKENS] == 75
    # Cache and reasoning fields should not be present
...
def test_openai_cache_tokens_with_reasoning(sentry_init, capture_events):
    """Test that both cached tokens and reasoning tokens can be tracked together."""
    sentry_init(
        integrations=[OpenAIIntegration(include_prompts=True)],
        traces_sample_rate=1.0,
        send_default_pii=True,
    ...
    # Response with both cached tokens and reasoning tokens
    mock_response = ChatCompletion(
        id="chatcmpl-test",
        model="o1-preview",
        object="chat.completion",
    ...
    # Add both prompt and completion token details
    mock_response.usage.prompt_tokens_details = {"cached_tokens": 150}
    mock_response.usage.completion_tokens_details = {"reasoning_tokens": 75}
    ...
    # Check both cached and reasoning tokens
    assert span["data"][SPANDATA.GEN_AI_USAGE_INPUT_TOKENS] == 200
    assert span["data"][SPANDATA.GEN_AI_USAGE_OUTPUT_TOKENS] == 100
    assert span["data"][SPANDATA.GEN_AI_USAGE_INPUT_TOKENS_CACHED] == 150
    assert span["data"][SPANDATA.GEN_AI_USAGE_OUTPUT_TOKENS_REASONING] == 75
