...
def test_websocket_auto_session_tracking_with_aggregates(
    app, sentry_init, capture_envelopes, request
):
    sentry_init(debug=True, send_default_pii=True)
    ...
    def failing_message():
        msg = "This is a failing message"
        raise ValueError(msg)

    def message():
        msg = "This is a working message"
        return msg
    ...
    async def app(scope, receive, send):
        ...
        await websocket.send_text(message())
        await websocket.send_text(message())
        await websocket.send_text(failing_message())
        await websocket.close()
    ...
    assert count_item_types["transaction"] == 0
    assert count_item_types["event"] == 1
    assert count_item_types["sessions"] == 1
