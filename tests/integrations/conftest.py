...
def capture_exceptions(monkeypatch):
    def inner():
        errors = set()
        old_capture_event = sentry_sdk.Scope.capture_event
        ...
        def capture_event(self, event, hint=None, scope=None):
            if hint:
                if "exc_info" in hint:
                    ...
                    errors.add(error)
            return old_capture_event(self, event, hint=hint, scope=scope)

        monkeypatch.setattr(sentry_sdk.Scope, "capture_event", capture_event)
