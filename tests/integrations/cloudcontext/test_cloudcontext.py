...
from sentry_sdk.integrations.cloudcontext import CLOUD_PLATFORM, CLOUD_PROVIDER
...
def test_is_aws_http_error():
    from sentry_sdk.integrations.cloudcontext import CloudContextIntegration

    response = MagicMock()
    response.status = 405

    CloudContextIntegration.http = MagicMock()
    CloudContextIntegration.http.request = MagicMock(return_value=response)

    assert CloudContextIntegration._is_aws() is False
    assert CloudContextIntegration.aws_token == ""
...
def test_is_aws_ok():
    ...
    assert CloudContextIntegration._is_aws() is True
    assert CloudContextIntegration.aws_token == b"something"

    CloudContextIntegration.http.request = MagicMock(side_effect=Exception("Test"))
    assert CloudContextIntegration._is_aws() is False
...
def test_get_aws_context(http_status, response_data, expected_context):
    ...
    assert CloudContextIntegration._get_aws_context() == expected_context
...
def test_is_gcp_http_error():
    ...
    assert CloudContextIntegration._is_gcp() is False
    assert CloudContextIntegration.gcp_metadata is None
...
def test_is_gcp_ok():
    ...
    assert CloudContextIntegration._is_gcp() is True
    assert CloudContextIntegration.gcp_metadata == {"some": "json"}
...
def test_get_gcp_context(http_status, response_data, expected_context):
    ...
    CloudContextIntegration.gcp_metadata = None
    ...
    assert CloudContextIntegration._get_gcp_context() == expected_context
...
def test_get_cloud_provider(is_aws, is_gcp, expected_provider):
    ...
    CloudContextIntegration._is_aws = MagicMock(return_value=is_aws)
    CloudContextIntegration._is_gcp = MagicMock(return_value=is_gcp)

    assert CloudContextIntegration._get_cloud_provider() == expected_provider
...
def test_get_cloud_context_unsupported_providers(cloud_provider):
    from sentry_sdk.integrations.cloudcontext import CloudContextIntegration

    CloudContextIntegration._get_cloud_provider = MagicMock(return_value=cloud_provider)

    assert CloudContextIntegration._get_cloud_context() == {}
...
def test_get_cloud_context_supported_providers(cloud_provider):
    from sentry_sdk.integrations.cloudcontext import CloudContextIntegration
...
@pytest.mark.parametrize(
    "cloud_provider, cloud_context, warning_called, set_context_called",
    [
        ["", {}, False, False],
...
def test_setup_once(cloud_provider, cloud_context, warning_called, set_context_called):
    from sentry_sdk.integrations.cloudcontext import CloudContextIntegration

    CloudContextIntegration.cloud_provider = cloud_provider
    CloudContextIntegration._get_cloud_context = MagicMock(return_value=cloud_context)

    with mock.patch(
        "sentry_sdk.integrations.cloudcontext.set_context"
    ) as fake_set_context:
        with mock.patch(
            "sentry_sdk.integrations.cloudcontext.logger.warning"
        ) as fake_warning:
            CloudContextIntegration.setup_once()

            if set_context_called:
                fake_set_context.assert_called_once_with("cloud", cloud_context)
            else:
