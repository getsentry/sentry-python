...
def capture_spotlight_envelopes(monkeypatch):
    def inner():
        envelopes = []
        test_spotlight = sentry_sdk_alpha.get_client().spotlight
...
def test_spotlight_off_by_default(sentry_init):
    sentry_init()
    assert sentry_sdk_alpha.get_client().spotlight is None


def test_spotlight_default_url(sentry_init):
    sentry_init(spotlight=True)

    spotlight = sentry_sdk_alpha.get_client().spotlight
