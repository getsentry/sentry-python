...
def test_dsc_head_of_trace_span_streaming(sentry_init, capture_envelopes):
    """
    Our service is the head of the trace (it starts a new trace)
    and sends a segment to Sentry.
    """
    sentry_init(
        dsn="https://mysecret@o1234.ingest.sentry.io/12312012",
        release="myapp@0.0.1",
    ...
    segment_envelope = envelopes[0]
    envelope_trace_header = segment_envelope.headers["trace"]
...
def test_dsc_head_of_trace_uses_custom_org_id_span_streaming(
...
def test_dsc_continuation_of_trace_span_streaming(sentry_init, capture_envelopes):
    """
    Another service calls our service and passes tracing information to us.
    Our service is continuing the trace and sends a segment to Sentry.
    """
    sentry_init(
        dsn="https://mysecret@o1234.ingest.sentry.io/12312012",
        release="myapp@0.0.1",
...
def test_dsc_continuation_of_trace_sample_rate_changed_in_traces_sampler_span_streaming(
    sentry_init, capture_envelopes
):
    """
    Another service calls our service and passes tracing information to us.
...
def test_dsc_issue_span_streaming(sentry_init, capture_envelopes):
    """
    Our service is a standalone service that does not have tracing enabled. Just uses Sentry for error reporting.
    """
    sentry_init(
...
def test_dsc_issue_with_tracing_span_streaming(sentry_init, capture_envelopes):
    """
    Our service has tracing enabled and an error occurs in a segment.
    Envelopes containing errors also have the same DSC as the segment envelopes.
    """
    sentry_init(
        dsn="https://mysecret@o1234.ingest.sentry.io/12312012",
        release="myapp@0.0.1",
    ...
    error_envelope, segment_envelope = envelopes

    assert error_envelope.headers["trace"] == segment_envelope.headers["trace"]
...
@pytest.mark.parametrize(
    ...
    [
        0,  # no traces will be started, but if incoming traces will be continued (by our instrumentations, not happening in this test)
        None,  # no tracing at all. This service will never create segments.
    ],
)
def test_dsc_issue_twp_span_streaming(
    sentry_init, capture_envelopes, traces_sample_rate
):
    """
    Our service does not have tracing enabled, but we receive tracing information from an upstream service.
    Error envelopes still contain a DSC. This is called "tracing without performance" or TWP for short.
