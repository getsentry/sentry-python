...
class FakeTransport(Transport):
    def __init__(self):
        self.options = {"dsn": "https://foo@sentry.io/123"}
        self.parsed_dsn = Dsn("https://foo@sentry.io/123")
    ...
    def flush(self, timeout, callback=None):
        self.flushed.append((timeout, callback))

    def kill(self):
        self.killed = True

    def record_lost_event(self, reason, data_category=None, item=None, *, quantity=1):
        self.lost_events.append((reason, data_category, item, quantity))
...
def _make_json_envelope():
    envelope = Envelope(headers={"event_id": "abc123"})
    envelope.add_item(Item(payload='{"message": "hello"}', type="event"))
    return envelope


def _make_binary_envelope():
    envelope = Envelope(headers={"event_id": "bin456"})
    envelope.add_item(Item(payload=b"\x00\x01\x02\x03", type="attachment"))
    return envelope
...
class Test_EnvelopePrinterTransportDelegation:
    def test_capture_envelope_delegates(self):
        ...
        assert inner.captured_envelopes == [envelope]
    ...
    def test_record_lost_event_delegates(self):
        ...
        assert inner.lost_events == [("queue_overflow", "error", item, 5)]
...
def _collect_debug_log_text(mock_debug):
    parts = []
...
class Test_EnvelopePrinterTransportLogging:
    def test_json_payload_is_pretty_printed(self):
        ...
        log_text = _collect_debug_log_text(mock_logger.debug)
        assert '"message": "hello"' in log_text
    ...
    def test_binary_payload_shows_byte_count(self):
        ...
        log_text = _collect_debug_log_text(mock_logger.debug)
        assert "<binary 4 bytes>" in log_text
    ...
    def test_item_type_and_headers_are_logged(self):
        ...
        log_text = _collect_debug_log_text(mock_logger.debug)
        assert "Item type:" in log_text
        assert "event" in log_text
        assert "Item headers:" in log_text
    ...
    def test_logging_exception_does_not_propagate(self):
        ...
        with mock.patch(
            "sentry_sdk.transport.json.dumps", side_effect=RuntimeError("boom")
        ):
            transport.capture_envelope(envelope)
    ...
    def test_empty_envelope_logs_headers_only(self):
        ...
        log_text = _collect_debug_log_text(mock_logger.debug)
        assert "empty1" in log_text
        assert "Item type:" not in log_text
        assert inner.captured_envelopes == [envelope]
    ...
    def test_multiple_items_each_logged(self):
        ...
        log_text = _collect_debug_log_text(mock_logger.debug)
        assert '"a": 1' in log_text
        assert "<binary 1 bytes>" in log_text


class TestMakeTransportEnvelopePrinter:
    def test_env_var_wraps_transport(self):
        options = {
            "dsn": "https://foo@sentry.io/123",
            "transport": None,
            "_experiments": {},
    ...
    def test_no_env_var_returns_plain_transport(self):
        ...
        with mock.patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SENTRY_PRINT_ENVELOPES", None)
            transport = make_transport(options)
    ...
    def test_falsy_env_var_values_do_not_wrap(self, value):
        ...
        with mock.patch.dict(os.environ, {"SENTRY_PRINT_ENVELOPES": value}):
            transport = make_transport(options)
