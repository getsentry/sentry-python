...
from ._compat import urlparse, text_type, implements_str, number_types
...
class Dsn(object):
    ...
    def to_auth(self, client=None):
        ...
        return Auth(scheme=self.scheme,
                    host=self.netloc,
...
class Auth(object):
    ...
    def __init__(self, scheme, host, project_id, public_key, secret_key=None,
                 version=7, client=None):
        self.scheme = scheme
        self.host = host
...
def stacktrace_from_traceback(tb, with_locals=True):
    return {
        'frames': [frame_from_traceback(tb, with_locals) for tb in iter_stacks(tb)]
    }
...
def exceptions_from_error_tuple(exc_type, exc_value, tb, with_locals=True):
    ...
    while exc_type is not None:
        rv.append(single_exception_from_error_tuple(exc_type, exc_value, tb,
                                                    with_locals))
