...
def invoke_agent_span(
    ...
    if should_send_default_pii():
        ...
        if original_input is None and explicit_instructions is not None:
            set_data_normalized(
                span,
        ...
        system_instructions = _get_system_instructions(original_input)
        ...
        if isinstance(original_input, str):
            ...
            messages = [
                message
                for message in original_input
                if not _is_system_instruction(message)
            ]
        else:
            messages = [
                {
